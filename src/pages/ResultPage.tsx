...
export function ResultPage() {
  ...
  return <div className={`result-page${isOwnResult ? ' is-own-result' : ''}`}>
    ...
    <ManualCard result={result} afterIntro={<>
      <div className="result-quick-actions"><Link className="button button-primary" to={`/share/${routeVersion}/${result.id}`} state={{ completed: isOwnResult }}>儲存／分享這份說明書</Link>{!isOwnResult && <Link className="text-link" to="/">我也想測測看</Link>}</div>
      <ResultContents isOwnResult={isOwnResult} />
    </>} />
    {isOwnResult && <AnswerReflection answers={reflectAnswers(quiz, state.answers, result.id)} />}
    <ResultChapters resultId={result.id} isOwnResult={isOwnResult} />
