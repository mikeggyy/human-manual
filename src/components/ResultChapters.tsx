...
export function ResultChapters({ resultId }: { resultId: string }) {
  ...
  return <div className="result-chapters">
    <section className="life-chapter" aria-labelledby="life-title">
      ...
      <p className="chapter-intro">把這個角色放進三個日常片段，看看哪些像你，哪些想換個寫法。</p>
    ...
    <div className="care-chapters">
      <section className="misreading-chapter" aria-labelledby="misreading-title">
        ...
        <div className="friend-note"><h3>可以這樣告訴朋友</h3><p>「{story.friendNote}」</p></div>
      </section>
    ...
    <section className="closing-letter" aria-labelledby="letter-title"><h2 id="letter-title">給你的最後一頁</h2><div>{story.letter.map((paragraph) => <p key={paragraph}>{paragraph}</p>)}<p className="letter-signoff">這是這一次的你。下一次，也可以有新的樣子。</p></div></section>
  </div>
}
