...
for (const width of [375, 768, 1440]) {
  test(`${width}px：開始、作答、返回修改、結果、分享 fallback 與重測`, async ({ page }) => {
    ...
    await page.goto('./')
    await expect(page).toHaveTitle('人類使用說明書｜出廠時沒附的那本，現在補給你。')
    await expect(page.getByRole('heading', { level: 1 })).toContainText('出廠時沒附的那本')
    await expect(page.locator('.book-scene')).toHaveAttribute('data-status', 'ready')
    await capture('home')
    ...
    await expect(page).toHaveTitle(`${type.name}｜人類使用說明書`)
    ...
    await expect(page.getByRole('heading', { name: '這次選擇留下的線索' })).toBeVisible()
    await page.getByText('看完整作答', { exact: false }).click()
    await expect(page.locator('.answer-recap li')).toHaveCount(8)
    for (let index = 0; index < 8; index++) {
      await expect(page.locator('.answer-recap li').nth(index).locator('p')).toHaveText(quiz.questions[index]!.options[index === 0 ? 1 : 0]!.label)
    }
    await page.getByText('看完整作答', { exact: false }).click()
    await page.getByRole('button', { name: '充電提案', exact: true }).click()
    await expect(page.getByRole('heading', { name: '今天的小小充電提案' })).toBeFocused()
    await capture('result')
    if (screenshotDir) await page.locator('.result-chapters').screenshot({ path: join(screenshotDir, `chapters-${width}.png`), animations: 'disabled' })
    await page.getByRole('link', { name: '分享說明書', exact: true }).click()
    await expect(page).toHaveTitle(`分享・${type.name}｜人類使用說明書`)
...
test('所有角色有完整結果；未知版本或角色可以回首頁', async ({ page }) => {
  for (const result of quiz.types) {
    ...
    const story = resultStories[result.id]!
    for (const scene of story.scenes) await expect(page.getByRole('heading', { name: scene.title, exact: true })).toBeVisible()
    await expect(page.getByText(story.letter[1]!, { exact: true })).toBeVisible()
    await expect(page.locator('.recharge-chapter li')).toHaveCount(3)
    await expect(page.locator('.answer-reflection')).toHaveCount(0)
    await expect(page.locator('main')).not.toContainText(/\bAI\b|ChatGPT|OpenAI|LLM|生成工具|人工智慧/)
  }
